...
plot_wal <- function(mods, dat, length_to_plot = NULL, date_range = c(NA, NA), 
  ...
  # --- ADDED: Check if CI limits actually exist to prevent blank ribbons and orphaned legends ---
  ...
  # Optional CI Ribbon (Now completely gated by the has_CI check)
  if(has_CI) {
    if(length(mods) == 1) {
  ...
  if(length(mods) == 1) {
    p <- p + 
      geom_line(aes(y = pred_wal, color = month), linewidth = 0.8) +
      geom_text(aes(y = pred_wal, label = month, color = month), size = 3, check_overlap = TRUE) +
      scale_color_gradient2(low = "darkblue", mid = "red", high = "darkgreen", midpoint = 7, name = "Month")
  } else {
    p <- p + 
